...

    {
        ...
        var generateComponentTrace = function (vm) {
            if (vm._isVue && vm.$parent) {
                ...
                return '\n\nfound in\n\n' + tree
                    .map(function (vm, i) {
                        return ("" + (i === 0 ? '---> ' : repeat(' ', 5 + i * 2)) + (Array.isArray(
                            vm)
                                                                                     ? ((formatComponentName(
                                vm[0])) + "... (" + (vm[1]) + " recursive calls)")
                                                                                     : formatComponentName(
                                vm)));
                    })
                    .join('\n')
...
// without getting yelled at by flow
    ...
    function initRender(vm) {
        ...
        var parentVnode = vm.$vnode = vm.$options._parentVnode; // the placeholder node in parent
                                                                // tree
...
// so they need to be treated as different nodes
    ...
    function addDirective(
        ...
        (el.directives || (el.directives = [])).push(
            {name: name, rawName: rawName, value: value, arg: arg, modifiers: modifiers});
    }
    ...
    /**
     * parse directive model to do the array update transform. a[idx] = val => $$a.splice($$idx, 1,
     * val)
     *
        ...
        parseHTML(template, {
            ...
            chars: function chars(text) {
                ...
                if (text) {
                    ...
                    } else if (text !== ' ' || !children.length || children[children.length
                                                                            - 1].text !== ' ') {
                        children.push({
                                          type: 3,
                                          text: text
...
// the listener for .once
    ...
    function bind$1(el, dir) {
        el.wrapData = function (code) {
            return ("_b(" + code + ",'" + (el.tag) + "'," + (dir.value) + "," + (dir.modifiers
                                                                                 && dir.modifiers.prop
                                                                                 ? 'true' : 'false')
...
// v-once
    ...
    function genData$2(el, state) {
        ...
        if (el.model) {
            data +=
                "model:{value:" + (el.model.value) + ",callback:" + (el.model.callback)
    ...
    function genDirectives(el, state) {
        ...
        for (i = 0, l = dirs.length; i < l; i++) {
            ...
            if (needRuntime) {
                hasRuntime = true;
                res +=
                    "{name:\"" + (dir.name) + "\",rawName:\"" + (dir.rawName) + "\"" + (dir.value
                                                                                        ? (",value:("
                    + (dir.value) + "),expression:" + (JSON.stringify(dir.value))) : '') + (dir.arg
                                                                                            ? (",arg:\""
                    + (dir.arg) + "\"") : '') + (dir.modifiers ? (",modifiers:" + (JSON.stringify(
                    dir.modifiers))) : '') + "},";
            }
        }
        if (hasRuntime) {
