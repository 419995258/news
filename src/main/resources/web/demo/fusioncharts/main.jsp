...
function validateTable(statTime){
	...
	if(statTime != ""){
		var myTable = "STAT_BASE_TEACHER_INFO_" + $.trim(statTime.split("-")[0].substring(2,4)) + "_" + $.trim(statTime.split("-")[1]);
		$.ajax({
			type:'post',
			url:'${root}/base/BaseSchool/isExistsTheTable.jspx',
			data:{"myTable":myTable},
			error: function(XMLHttpRequest, textStatus, errorThrown) {
						 $.dialog.tips('XMLHttpRequest.status：'+XMLHttpRequest.status+',XMLHttpRequest.readyState:'+XMLHttpRequest.readyState+',textStatus:'+textStatus,2,'loading.gif');
				   },
			async:false,
...
function exportExcel(){
	var countryToSchoolStatus = $("#countryToSchool").val();
	if(countryToSchoolStatus == 'true'){
		window.location.href="${root}/base/BaseKpiItem/countryTeacherIndexSchool.jspx?menu=${menu}&schoolId=${schoolId}&sffsbjs=${sffsbjs}&TJSJ=${TJSJ}&areaId=${areaId}&areaCode=${areaCode}&subareaId=${subareaId}&isExcel=1";
	}else{
		window.location.href="${root}/base/BaseKpiItem/countryTeacherIndex.jspx?menu=${menu}&sffsbjs=${sffsbjs}&TJSJ=${TJSJ}&areaId=${areaId}&areaCode=${areaCode}&subareaId=${subareaId}&isExcel=1";
	}
}
function exportPdf(){
	var countryToSchoolStatus = $("#countryToSchool").val();
	if(countryToSchoolStatus == 'true'){
		window.location.href="${root}/base/BaseKpiItem/exportPDF.jspx?menu=${menu}&schoolId=${schoolId}&sffsbjs=${sffsbjs}&TJSJ=${TJSJ}&areaId=${areaId}&areaCode=${areaCode}&subareaId=${subareaId}&isPdf=1&leftmenu=countrySchool";
	}else{
		window.location.href="${root}/base/BaseKpiItem/exportPDF.jspx?menu=${menu}&sffsbjs=${sffsbjs}&TJSJ=${TJSJ}&areaId=${areaId}&areaCode=${areaCode}&subareaId=${subareaId}&isPdf=1&leftmenu=countryMain";
	}
}
...
<!-- countryTeacher/main.jsp -->
	...
	<div class="aside">
        <ul>
            <li <c:if test="${menuFlag_2 eq 'index'}">class="on"</c:if>><a href="#" class="a3" onclick="changeUrl('${root}/base/BaseKpiItem/countryTeacherIndex.jspx?FXZBSelect=all&menu=${SSXD }',this,'${SSXD }');" style="cursor:pointer">基本情况</a></li>
	        <li  <c:if test="${menuFlag_2 eq 'money'}">class="on"</c:if>><a href="#" class="a4" onclick="changeUrl('${root}/base/BaseKpiItem/countryTeacherMoney.jspx?FXZBSelect=all&menu=${SSXD }',this,'${SSXD }');" style="cursor:pointer">基本待遇</a></li>
	        <li  <c:if test="${menuFlag_2 eq 'honor'}">class="on"</c:if>><a href="#" class="a5" onclick="changeUrl('${root}/base/BaseKpiItem/countryTeacherHonor.jspx?menu=${SSXD }',this,'${SSXD }');" style="cursor:pointer">教师荣誉</a></li>
        </ul>
      </div>
	<div class="main" style="padding-top: 40px;">
	 <div class="main_inner">
		...
		<input type="hidden" name="countryToSchool" id="countryToSchool" value="${countryToSchool }">
		...
		<input type="hidden" name="FXZBSelect" id="FXZBSelect" value="${FXZBSelect }">
		 ...
		 <div class="title">筛选条件<!-- <a href="#" class="add"></a> --><span class="pot"></span></div>
		    ...
		    <div class="btn">
				...
				<ul>
                        <li>
                            <label>分析指标：</label>
                             <p>
                             <label class="ck_model"><span><input name="FXZB"  type="checkbox" id="allCb_FXZB"  value="all"  onclick="checkAll2('FXZB');" ${all}></span>所有</label>
                             <label class="ck_model"><span><input  type="checkbox" id="SFZBCheck" name="FXZB" value="SFZB" onclick="everyCheckboxClick(this);" ${SFZB}/></span>在编</label>
                             <label class="ck_model"><span><input  type="checkbox" id="SFXJJYSGGJSCheck" name="FXZB" value="SFXJJYSGGJS" onclick="everyCheckboxClick(this);"  ${SFXJJYSGGJS}/></span>骨干</label>
                             <label class="ck_model"><span><input  type="checkbox" id="XRGWDJCheck" name="FXZB" value="XRGWDJ" onclick="everyCheckboxClick(this);"  ${XRGWDJ}/></span>岗位级别</label>
							</p>
                        </li>
                        <li>
                            <label>统计时间：</label>
								...
								<select name="TJSJ" id="TJSJ">
									<option value="">最新</option>
										<c:forEach items="${my431fn:getDatesFromSomeDate()}" var="dat" >
											<option value="${dat }" <c:if test="${TJSJ eq dat}">selected = "selected"</c:if>>${dat }</option>
										</c:forEach>
								</select>
                                <span class="label">附设班：</span>
                             	 <select name="sffsbjs" style="width:200px">
									<option value="">全部</option>
									<option value="1" <c:if test="${sffsbjs eq 1}">selected="selected"</c:if>>是</option>
									<option value="0" <c:if test="${sffsbjs eq 0}">selected="selected"</c:if>>否</option>
									</select>
                            </p>
                        </li>
        		 </ul>
     ...
     <c:if test="${fn:startsWith(areaCode, wsAreaCode)}">
				<c:if test="${fn:length(areaCode)<=3}">
					<a href="###" style="margin-left: 120px;">${my431fn:getAreaByCodeOne(areaCode).areaName}</a>
				</c:if>
				<c:if test="${fn:length(areaCode)==6}">
					<c:if test="${fn:length(wsAreaCode)<=3}">
						<a href="###" onclick="toSubArea('${my431fn:getAreaByCodeOne(fn:substring(areaCode,0,3)).id}');" style="margin-left: 90px;">${my431fn:getAreaByCodeOne(fn:substring(areaCode,0,3)).areaName}</a> >
					</c:if>
				...
				<c:if test="${fn:length(areaCode)==9}">
					...
					<c:if test="${fn:length(wsAreaCode)<=6}">
						<a href="###" onclick="toSubArea('${my431fn:getAreaByCodeOne(fn:substring(areaCode,0,6)).id}');">${my431fn:getAreaByCodeOne(fn:substring(areaCode,0,6)).areaName}</a> >
					</c:if>
					<c:if test="${countryToSchool ne 'true'}">
					...
					<c:if test="${countryToSchool eq 'true'}">
						...
						<c:if test="${fn:length(wsAreaCode) ne 9}">
							<a href="###" onclick="toSubArea('${my431fn:getAreaByCodeOne(areaCode).id}')">${my431fn:getAreaByCodeOne(areaCode).areaName}</a> 
						</c:if>
					</c:if>
	...
	<a href="###" class="export exportexcel" style="margin-right: 10px;" onclick="exportExcel()"></a>
	<c:if test="${fn:startsWith(areaCode, wsAreaCode)}">
				<c:if test="${fn:length(areaCode)<=3}">
					<a href="###" style="margin-left: 120px;">${my431fn:getAreaByCodeOne(areaCode).areaName}</a>
				</c:if>
	...
	<%-- 当前地区：${areaName}&nbsp;&nbsp;&nbsp;&nbsp;
 	...
 	<div class="ex_table" style="overflow: visible;margin-top: 0px;border: 0px solid #dbdbdb;heigth:900px;;  ">
	  <table  id="MyTable">
			<thead>
				<tr>
					<th rowspan="3">序号</th>
					...
					<c:if test="${countryToSchool ne 'true'}">
						<c:if test="${SFZB !=null }"><th colspan="9">在编情况</th></c:if>
						<c:if test="${SFXJJYSGGJS !=null }"><th colspan="9">骨干教师</th></c:if>
						<c:if test="${XRGWDJ !=null }"><th colspan="9">岗位等级</th></c:if>
					</c:if>
					<c:if test="${countryToSchool eq 'true'}">
			...
			<c:if test="${countryToSchool ne 'true'}">
			 <c:set var="mapGet" value="${mapGet}"></c:set> 
					...
					<c:forEach items="${areaList}" var="dat" varStatus="index">
						<tr>
							<td>${index.count}</td>
							<td class="widthStand"><a href="###" onclick="toSubArea('${dat.id}');">${dat.nodeName}</a></td>
								<!-- 在编 -->
								<c:if test="${SFZB !=null }">
					...
					<c:if test="${fn:length(areaCode) eq 9 }">
						...
						<tr>
							...
							<td class="widthStand"><a href="###" onclick="countryToSchoolFirst('${schoolDat['SCHOOL_ID']}');">${schoolDat['SCHOOL_NAME']}</a></td>
								<!-- 在编 -->
								<c:if test="${SFZB !=null }">
...
<!-- 分析结果-->
       <div class="fixedResult" >
           ...
           <div class="showBox" style="height:400px;  overflow-y:auto; ">
               <h3>分析结果</h3>
              	<p>  从我省教师基本情况的统计来看，教师的在编比例方面，城区教师的在编率为<span class="blue">${my431fn:getPercentFloorV2(mapGet['TSFZB1Count'],mapGet['TCount'],"%.2f")}</span>，乡村教师的在编率为：<span class="blue">${my431fn:getPercentFloorV2(mapGet['CSFZB1Count'],mapGet['CCount'],"%.2f")}</span>；
               		 县级及县级以上骨干教师方面，城区教师骨干率为<span class="blue">${my431fn:getPercentFloorV2(mapGet['TSFXJJYSGGJS1Count'],mapGet['TCount'],"%.2f")}</span>，乡村教师的骨干率为<span class="blue">${my431fn:getPercentFloorV2(mapGet['CSFXJJYSGGJS1Count'],mapGet['CCount'],"%.2f")}</span>； 
               	 	现任岗位的专业技术等级方面，城区教师中高级比例占到教师岗位的<span class="blue">${my431fn:getPercentFloorV2(mapGet['TXRGWDJHCount'] + mapGet['TXRGWDJMCount'],mapGet['TCount'],"%.2f")}</span>，乡村教师中高级教师比例占到教师岗位的<span class="blue">${my431fn:getPercentFloorV2(mapGet['CXRGWDJHCount'] + mapGet['CXRGWDJMCount'],mapGet['CCount'],"%.2f")}</span>；
