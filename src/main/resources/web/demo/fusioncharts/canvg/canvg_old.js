...
(function () {
    ...
    this.canvg = function (target, s, opts) {
        ...
        if (!(target.childNodes.length == 1 && target.childNodes[0].nodeName
              == 'OBJECT')) {
            target.svg = svg;
        }
        svg.opts = opts;
    ...
    function build() {
        ...
        svg.Transform = function (v) {
            ...
            var data = svg.trim(svg.compressSpaces(v)).replace(/\)(\s?,\s?)/g, ') ')
                .split(/\s(?=[a-z])/);
            ...
            if (node != null && node.nodeType == 1) { //ELEMENT_NODE
                ...
                for (var i = 0; i < node.attributes.length; i++) {
                    ...
                    this.attributes[attribute.nodeName] =
                        new svg.Property(attribute.nodeName, attribute.nodeValue);
                }
        ...
        svg.Element.path = function (node) {
            ...
            d = d.replace(/([MmZzLlHhVvCcSsQqTtAa])([MmZzLlHhVvCcSsQqTtAa])/gm, '$1 $2'); // separate
                                                                                          ...
                                                                                          // from
                                                                                          // commands
            d = d.replace(/([MmZzLlHhVvCcSsQqTtAa])([^\s])/gm, '$1 $2'); // separate commands from
                                                                         // points
            d = d.replace(/([^\s])([MmZzLlHhVvCcSsQqTtAa])/gm, '$1 $2'); // separate commands from
                                                                         // points
        ...
        svg.Element.stop = function (node) {
            ...
            if (this.style('stop-opacity').hasValue()) {
                stopColor =
                    stopColor.addOpacity(this.style('stop-opacity').value);
            }
        ...
        svg.Element.text = function (node) {
            ...
            this.getBoundingBox = function () {
                // TODO: implement
                return new svg.BoundingBox(this.attribute('x').toPixels('x'),
                                           this.attribute('y').toPixels('y'), 0, 0);
            }
            ...
            this.renderChild = function (ctx, parent, i) {
                ...
                if (child.attribute('x').hasValue()) {
                    child.x =
                        child.attribute('x').toPixels('x') + this.getAnchorDelta(ctx, parent, i);
                } else {
            ...
            var draw = function () {
                ...
                if (ctx.canvas.parentNode) {
                    svg.ViewPort.SetCurrent(
                        ctx.canvas.parentNode.clientWidth, ctx.canvas.parentNode.clientHeight);
                }
                ...
                if (isFirstRender) {
                    isFirstRender = false;
                    if (typeof (svg.opts['renderCallback'])
                        == 'function') {
