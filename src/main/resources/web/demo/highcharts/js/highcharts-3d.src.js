...
////// CUBOIDS //////
    Highcharts.SVGRenderer.prototype.cuboid = function (shapeArgs) {
        ...
        result.front =
            this.path(paths[0]).attr({zIndex: paths[3], 'stroke-linejoin': 'round'}).add(result);
        result.top =
            this.path(paths[1]).attr({zIndex: paths[4], 'stroke-linejoin': 'round'}).add(result);
        result.side =
            this.path(paths[2]).attr({zIndex: paths[5], 'stroke-linejoin': 'round'}).add(result);
    ...
    Highcharts.SVGRenderer.prototype.cuboidPath = function (shapeArgs) {
        ...
        var z2 = (beta > 0 ? (pArr[0].z + pArr[7].z + pArr[6].z + pArr[1].z) / 4 : (pArr[3].z
                                                                                    + pArr[2].z
                                                                                    + pArr[5].z
                                                                                    + pArr[4].z)
                                                                                   / 4);
        ...
        var z3 = (alpha > 0 ? (pArr[1].z + pArr[2].z + pArr[5].z + pArr[6].z) / 4 : (pArr[0].z
                                                                                     + pArr[7].z
                                                                                     + pArr[4].z
                                                                                     + pArr[3].z)
                                                                                    / 4);
...
// Shorthand to check the is3d flag
    ...
    Highcharts.wrap(Highcharts.Axis.prototype, 'render', function (proceed) {
        ...
        if (this.horiz) {
            ...
            if (!this.bottomFrame) {
                this.bottomFrame =
                    renderer.cuboid(bottomShape).attr({
                                                          fill: fbottom.color,
                                                          zIndex: (chart.yAxis[0].reversed && alpha
                                                                   > 0 ? 4 : -1)
            ...
            if (!this.backFrame) {
                this.backFrame =
                    renderer.cuboid(backShape).attr({fill: fback.color, zIndex: -3})
            ...
            if (!this.sideFrame) {
                this.sideFrame =
                    renderer.cuboid(sideShape).attr({fill: fside.color, zIndex: -2})
    ...
    Highcharts.wrap(Highcharts.seriesTypes.column.prototype, 'translate', function (proceed) {
        ...
        Highcharts.each(series.data, function (point) {
            ...
            tooltipPos =
                perspective([{x: tooltipPos[0], y: tooltipPos[1], z: z}], alpha, beta, origin)[0];
                    ...
                    Highcharts.each(series.data, function (point) {
                        ...
                        if (!reversed) {
                            if (point.stackY) {
                                ...
                                point.shapeArgs.y =
                                    point.plotY + (point.negative ? -point.height : point.height);
                            }
                        }
                    });
        ...
        if (this.chart.is3d()) {
            ...
            if (!z) {
                if (!(grouping !== undefined && !grouping) && stacking) {
                    ...
                    z =
                        (stacks.totalStacks * 10) - (10 * (stacks.totalStacks
                                                           - stacks[stack].position)) - i;
    ...
    function draw3DPoints(proceed) {
        ...
        if (this.chart.is3d()) {
            ...
            Highcharts.each(this.data, function (point) {
                var c = point.options.borderColor || point.color
    ...
    Highcharts.wrap(Highcharts.seriesTypes.pie.prototype.pointClass.prototype, 'haloPath',
                    function (proceed) {
    ...
    Highcharts.wrap(Highcharts.seriesTypes.pie.prototype, 'drawPoints', function (proceed) {
        ...
        if (this.chart.is3d()) {
            ...
            Highcharts.each(this.data, function (point) {
                var c = point.options.borderColor || point.color
                        || point.series.userOptions.borderColor || point.series.color;
                point.options.borderColor = c;
                point.borderColor = c;
                point.pointAttr[''].stroke = c;
                // same bordercolor on hover and select
        ...
        Highcharts.each(series.data, function (point) {
            ...
            if (point.dataLabel) {
                point.dataLabel.attr({
                                         x: point.dataLabel.connX + (-r * (1 - cos(b1)) * cos(a2))
                                            + (cos(a2) > 0 ? cos(b1) * d : 0)
                                            - (point.dataLabel.width / 2),
                                         y: point.dataLabel.connY + (-r * (1 - cos(a1)) * sin(a2))
                                            + (sin(a2) > 0 ? sin(a1) * d : 0)
