                ...
                each(table.getElementsByTagName('tr'), function (tr, rowNo) {
                    if (rowNo >= startRow && rowNo <= endRow) {
                        each(tr.children, function (item, colNo) {
                            if ((item.tagName === 'TD' || item.tagName === 'TH') && colNo
                                >= startColumn && colNo <= endColumn) {
                                if (!columns[colNo - startColumn]) {
                                    columns[colNo - startColumn] = [];
                                }
                                ...
                                success: function (json) {
                                    ...
                                    for (i = 0; i < colCount; i++) {
                                        if (i >= startColumn && i <= endColumn) {
                                            ...
                                            columns[i - startColumn].length =
                                                Math.min(rowCount, endRow - startRow);
                                        }
                                    }
                                    ...
                                    for (i = 0; i < cellCount; i++) {
                                        ...
                                        // set the transposed cell value in the newly created
                                        // columns
                                        if (gc >= startColumn && gc <= endColumn &&
                                            gr >= startRow && gr <= endRow) {
                                            columns[gc - startColumn][gr - startRow] =
                                                cell.content.$t;
                                        }
                                    }
                                    self.dataFound();
        ...
        complete: function () {
            ...
            if (options.complete) {
                ...
                for (i = 0, seriesIndex = 0; i < columns.length; seriesIndex++) {
                    ...
                    valueCount =
                        Highcharts.pick(this.valueCount.individual[seriesIndex],
                    ...
                    if (userOptions.hasOwnProperty('series')) {
                        if (typeof userOptions.series === 'object') {
                            each(userOptions.series, function (series, i) {
                                userOptions.series[i] =
                                    Highcharts.merge(series, dataOptions.series[i]);
                            });
